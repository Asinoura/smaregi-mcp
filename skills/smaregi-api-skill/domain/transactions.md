...
- ⚠️ 31日を超える期間が指定された場合、executeツールが自動的に期間を分割してリクエストします（手動分割は不要）
- ⚠️ _aggregation.covered_period と covered_months で実際にカバーした期間を確認してください
- ⚠️ _aggregation.warning の内容を必ずユーザーに伝えてください。対象期間外のデータは含まれていません
- ⚠️ _aggregation.monthly_breakdown に月別の内訳があります。年間集計の場合はこれを使ってください
- ⚠️ _aggregation に含まれないデータを推測・補完してはいけない。取得できなかった期間がある場合は「データが取得できませんでした」と正直に伝えること
