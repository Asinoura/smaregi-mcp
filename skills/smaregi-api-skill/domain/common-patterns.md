...
31日を超える期間のデータが必要な場合、executeツールが自動的に分割取得します。手動で分割する必要はありません。

- ⚠️ _aggregation.warning が存在する場合、その内容を必ずユーザーに伝えること
- ⚠️ 集計値を報告する際は、必ず _aggregation.covered_period の対象期間を併記すること
- ⚠️ 対象期間外のデータについて推測や補完をしないこと（ハルシネーション防止）
- ⚠️ _aggregation.monthly_breakdown がある場合は、年間の月別内訳として使うこと（自分で計算しない）
